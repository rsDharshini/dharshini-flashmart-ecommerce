...
def initiate_payment(body, user):
    ...
    return response(201, {
        "payment_id":        payment_id,
        "razorpay_order_id": razorpay_order["id"],
        "razorpay_key_id":   RAZORPAY_KEY_ID,     # ✅ matches frontend
        "amount_paise":      amount_paise,          # ✅ matches frontend
        "currency":          currency
    })
...
def create_razorpay_order(amount, currency, receipt):
    ...
    try:
        ...
        error_body = e.read().decode()  # ← READ the actual Razorpay error
        return None, f"Razorpay error {e.code}: {error_body}"
    except Exception as e:
        return None, str(e)
